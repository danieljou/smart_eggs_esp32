...
String  API_BASE_URL =  "http://192.168.1.100:8000/";
...
void send_g_t_h_to_save(float gas, float temperature, float humidity ){
  HTTPClient http;
  String path = API_BASE_URL + "incubation_saved_data/" + INC_SERIAL_NUNBER + "/";
  http.begin(path);
  http.addHeader("Content-type","application/json");
...
//  http://127.0.0.1:8000/send_notification_movement/INC_001/
  ...
  http.begin("http://192.168.1.100:8000/send_notification_movement/INC_001/");
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    previousMillis = currentMillis;
      int sensorValue = detectAmmoniaGas();
      send_g_t_h_to_save(sensorValue ,temperature , humidity);
